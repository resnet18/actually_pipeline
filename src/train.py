"""
act2 预训练脚本（基于Common Crawl子集，通用语料）
...
def main():
    ...
    print(f"model: act2 | params={n_params:,}")
    ...
    try:
        for epoch in range(start_epoch, EPOCHS):
            ...
            for x, y in pbar:
                ...
                if step % SAVE_EVERY == 0:
                    latest = CKPT_DIR / "act2-latest.pt"
                    torch.save({
            ...
            ep_path = CKPT_DIR / f"act2-epoch{epoch+1}.pt"
            torch.save({
                "step": step, "epoch": epoch,
                "model_state_dict": model.state_dict(),
                "optimizer_state_dict": optimizer.state_dict(),
        ...
        interrupt = CKPT_DIR / "act2-interrupt.pt"
        torch.save({
            "step": step, "epoch": epoch,
            "model_state_dict": model.state_dict(),
            "optimizer_state_dict": optimizer.state_dict(),
