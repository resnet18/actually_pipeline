"""
act2 继续预训练脚本（ACL 领域语料）
用法: python src/train_cpt.py
从 model/act2-latest.pt 恢复，学习率更低
"""
import os
import json
import math
...
def main():
    ...
    print(f"model: act2-cpt | params={n_params:,}")
    ...
    try:
        for epoch in range(EPOCHS):
            ...
            for x, y in pbar:
                ...
                lr = get_lr(step, 50, total_steps, LR)  # warmup 更短
                ...
                if step % SAVE_EVERY == 0:
                    latest = CKPT_DIR / "act2-cpt-latest.pt"
                    torch.save({
                    ...
                    if loss.item() < best_loss:
                        best_loss = loss.item()
                        best = CKPT_DIR / "act2-cpt-best.pt"
                        torch.save({
                            "step": step, "epoch": epoch,
                            "model_state_dict": model.state_dict(),
                            "optimizer_state_dict": optimizer.state_dict(),
            ...
            ep_path = CKPT_DIR / f"act2-cpt-epoch{epoch+1}.pt"
            torch.save({
                "step": step, "epoch": epoch,
                "model_state_dict": model.state_dict(),
                "optimizer_state_dict": optimizer.state_dict(),
        ...
        interrupt = CKPT_DIR / "act2-cpt-interrupt.pt"
        torch.save({
            "step": step, "epoch": epoch,
            "model_state_dict": model.state_dict(),
            "optimizer_state_dict": optimizer.state_dict(),
